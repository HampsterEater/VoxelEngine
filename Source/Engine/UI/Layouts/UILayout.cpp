...
UIElement* UILayout::Instantiate_Element(const UILayoutElementDefinition* def) const
{
...
#define ELEMENT_END() \
				...
				{ \
					element->Add_Child(Instantiate_Element(*iter)); \
				} \
				return element;	\
			} \
		}
...
std::vector<UIElement*> UILayout::Instantiate() const
{
	...
	{
		UIElement* element = Instantiate_Element(*iter);
...
#define ELEMENT_PROPERTY(name, fieldname, type, required)	\
	if (file->Contains(#name, root_node, true)) \
		...
		{ \
			...
			if (required) \
			{ \
				DBG_LOG("Element does not contain required property '%s'.", #name); \
				SAFE_DELETE(state); \
				return false; \
			} \
		}
...
#undef ELEMENT_START
	...
	if (max_children >= 0 && result->Children.size() > max_children)
	{
		DBG_LOG("Element '%s' contains to many children, max children allowed for this element is %i.", type, max_children);
		return false;
	}
	if (force_max_children == true && result->Children.size() != max_children)
	{
